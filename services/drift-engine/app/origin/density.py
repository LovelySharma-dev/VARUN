...
def calculate_origin_density(
    ...
    # Add padding
    lat_pad = grid_resolution_km / 111.0  # ~111 km per degree
    lon_pad = grid_resolution_km / (111.0 * np.cos(np.radians((lat_min + lat_max) / 2)))

    lat_edges = np.arange(lat_min - lat_pad, lat_max + lat_pad, grid_resolution_km / 111.0)
    lon_edges = np.arange(lon_min - lon_pad, lon_max + lon_pad, grid_resolution_km / (111.0 * np.cos(np.radians((lat_min + lat_max) / 2))))
    ...
    hist_smooth = gaussian_filter(hist.T, sigma=sigma_grid)

...
def health():
    try:
        ...
        return {
            ...
            "scaler_feature_count": int(package.scaler.n_features_in_),
            "reconstruction_error_threshold": package.threshold,
        }
    except Exception:
        return {
            "status": "ok",
...
def analyze_ais(request: AISAnalysisRequest):
    try:
        ...
        return {
            ...
            "preprocessing_audit": result.preprocessing_audit,
            "inference_audit": result.inference_audit,
            "dark_gap_event_count": len(result.dark_gap_events),
            "top_vessels": top_vessels,
        }
    except (FileNotFoundError, NotADirectoryError, ValueError, KeyError) as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc


@app.post("/rank-candidates")
def rank_phase3_candidates(request: CandidateRankingRequest):
    try:
        ...
        return {
            ...
            "model_version": result.model_package.metadata["model_version"],
            "candidate_count": result.ranking.audit["candidate_count"],
            "scored_windows": result.inference_audit["scored_windows"],
            "dark_gap_event_count": len(result.dark_gap_events),
            "score_semantics": result.ranking.audit["score_semantics"],
            "calibration_status": result.ranking.audit["normalization"]["calibration_status"],
...
def run_phase3(request: Phase3RunRequest):
    ...
    except (FileExistsError, FileNotFoundError, NotADirectoryError, ValueError, KeyError) as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
def run_phase3_public(request: Phase3RunPublicRequest) -> Phase3RunPublicResponse:
    ...
    try:
        ...
        return Phase3RunPublicResponse.model_validate(response_payload)
    except (FileExistsError, FileNotFoundError, NotADirectoryError, ValueError, KeyError) as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
def validate_phase2(request: Phase2FolderRequest):
    try:
        contract = load_phase2_bundle(Path(request.phase2_folder))
        ...
        return {
            ...
            "release_window": contract.release_window.model_dump(mode="json"),
